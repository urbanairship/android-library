...
public class LayoutEventContextTest {
    ...
    public fun testJSON() {
        val context = LayoutEventContext(
            pager = LayoutEventContext.Pager(
                identifier = "pager id",
            ...
            button = LayoutEventContext.Button(
                identifier = "button id"
            ),
            form = LayoutEventContext.Form(
                identifier = "form id", submitted = true, type = "form type"
            ),
            reportingContext = JsonValue.wrap("reporting context"),
            experimentReportingData = listOf(
                jsonMapOf("experiment 1" to "result"), jsonMapOf("experiment 2" to "result")
            )
        )
    ...
    public fun testMake() {
        ...
        val displayContext = LayoutEventContext.Display(
        ...
        val expected = LayoutEventContext(
            pager = LayoutEventContext.Pager(
                identifier = "pager-id",
                pageIndex = 1,
                pageIdentifier = "page-id",
            ...
            button = LayoutEventContext.Button("button-identifier"),
            reportingContext = reportingMetadata,
