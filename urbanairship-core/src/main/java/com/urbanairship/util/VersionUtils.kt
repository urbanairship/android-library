...
public object VersionUtils {
     ...
     */
    public fun createVersionObject(appVersion: Long): JsonSerializable {
        // Get the version code
        return jsonMapOf(getPlatformName() to jsonMapOf(VERSION_KEY to appVersion)).toJsonValue()
    }
     ...
     */
    public fun createVersionPredicate(versionMatcher: ValueMatcher): JsonPredicate {
        return JsonPredicate
            .newBuilder()
            .addMatcher(
                matcher = JsonMatcher.newBuilder()
                    .setScope(getPlatformName())
                    .setKey(VERSION_KEY)
