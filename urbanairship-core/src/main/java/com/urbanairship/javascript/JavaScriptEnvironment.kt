...
public class JavaScriptEnvironment private constructor(builder: Builder) {
     ...
     * access to it.
     *
     * Note: This needs to be `_UAirship` to match the content of `ua_native_bridge`.
     * We alias `Airship = UAirship` in the native bridge, so consumers can use either
     * entry point (new or legacy).
     *
     * @hide
     */
    @WorkerThread
    ...
    public class Builder {
        ...
        public fun addGetter(functionName: String, value: JsonSerializable): Builder {
            ...
            val getter = "_UAirship.$functionName = function(){return $json;};"
            getters.add(getter)
            return this
        }
