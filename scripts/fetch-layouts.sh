...
# urbanairship/thomas-layouts repo into the devapp assets directory.
#
...
# structure the devapp expects: sample_layouts/Scenes/{Modal,Banner,Embedded}.
# The in-app *Message* fixtures (sample_layouts/Messages) are tracked directly
# in this repo and are NOT fetched here.
#
...
#                   and exit 0. The devapp still builds (Scene list just shows
...
DEST="${ROOT_PATH}/devapp/src/main/assets/sample_layouts"
...
fail_or_warn() {
  ...
  echo "warning: fetch-layouts: the devapp will still build, but the Thomas Scene list may be empty. Re-run './gradlew :devapp:fetchLayouts' once you have access." >&2
  exit 0
}
...
# Scenes/ keeps a tracked .gitkeep so the directory survives a fresh clone; its
# fetched contents are git-ignored. We clear only the fetched contents, then
# copy the new ones in. (Dot-files in assets are excluded from the APK by
# AAPT's default ignore pattern, so .gitkeep and the marker are never packaged.)
mkdir -p "$DEST"
