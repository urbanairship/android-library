...
- 32 is a ceiling rather than a choice. Robolectric 4.16's native graphics has no text measurement above 32: 33 and 34 both die with `UnsatisfiedLinkError: MeasuredText.nGetExtent`. It is at least past `Build.VERSION_CODES.R`, so the renderer takes its modern `currentWindowMetrics` path instead of the pre-30 `displayMetrics` fallback.
