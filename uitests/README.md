...
- The device configuration is pinned in `config.json`: `robolectric.sdk` 32 and `robolectric.qualifiers` `w411dp-h891dp-xhdpi`, so every capture is an 822x1782 px phone. Both values are part of the baseline identity — the baseline label is `robolectric-<sdk>` (`robolectric-32`) and the artifact is `baselines-robolectric-<sdk>`. Changing either invalidates every baseline, which is exactly why they are pinned rather than defaulted. The level also has to match `@Config(sdk = ...)` on `SceneScreenshotTest`, which is the one thing `config.json` cannot drive, since an annotation needs a compile-time constant.
- 32 is a ceiling rather than a choice, and it belongs to Robolectric rather than to Roborazzi. 4.16's native graphics has no text measurement above 32: 33, 34 and 36 all die with `UnsatisfiedLinkError: MeasuredText.nGetExtent`, whichever Roborazzi version is in use. Robolectric 4.17 does render at 35, but it fails roughly 1469 of this repo's existing tests across nearly every module, so lifting the ceiling is a repo-wide migration and not something this suite can do on its own. 32 is at least past `Build.VERSION_CODES.R`, so the renderer takes its modern `currentWindowMetrics` path instead of the pre-30 `displayMetrics` fallback.
