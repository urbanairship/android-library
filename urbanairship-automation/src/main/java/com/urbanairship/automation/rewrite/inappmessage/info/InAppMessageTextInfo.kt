    ...
    public companion object {
        ...
        public fun fromJson(source: JsonValue): InAppMessageTextInfo {
            ...
            return InAppMessageTextInfo(
                ...
                color = content.get(COLOR_KEY)?.let(InAppMessageColor::fromJson),
                size = content.opt(SIZE_KEY).getFloat(0F),
                alignment = content.optionalField<String>(ALIGNMENT_KEY)?.let(Alignment::fromJson),
                style = content.get(STYLE_KEY)?.requireList()?.map(Style::fromJson),
